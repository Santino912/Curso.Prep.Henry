...
if(!objeto[string[property]]){
objeto[string[property]] = 1;
}else{
  objeto[string[property]] += 1;
  }
  }
...
function asAmirror(str) {
  ...
  for(let i = 0; i < str.length; i++){
    for(let o = str[i].length-1; o >= 0; o--){
    stringArray.unshift(str[i][o]);
  }
  stringArray.unshift(" ");
  }
  stringArray.shift();
...
function capicua(numero){
  ...
  //Escribe tu código aquí
  let numeroRev;
  numeroRev = numero.toString().split("").reverse().join("");
  if(numeroRev == numero){
    return "Es capicua";
...
function buscoInterseccion(arreglo1, arreglo2){
  ...
  arreglo1.forEach(elemento => {
    if(arreglo2.indexOf(elemento) >= 0){
      array.push(elemento);
    }
  });
  return array;
